...
double memory_used(const std::vector<std::map<std::string, std::vector<bool> > >& level_operator_timeslice_defined, int T, int L) {
	...
	return timeslice_num * bytes_per_timeslice + 2 * gauge_field_bytes;
}
...
int main(int argc, char** argv) {
	...
	if (show_mem) {
		cout << "This computation uses "
				<< memory_used(level_operator_timeslice_defined, T, L) / 1000000.0
				<< " MB memory.\n";
		return 0;
	}
...
//	***************************************************************************************************************************************
	...
	MultilevelAnalyzer multilevel(multilevel_config, WL_R, level_operator_factors, level_operator_timeslice_defined,
			lowest_level_functions);

	map<string, T_field> T_fields;
	try {
...
//	***************************************************************************************************************************************
	...
	for (const auto& name_field : T_fields) {
		const string& fieldname = name_field.first;
		complex WL_avg;
		co_eq_zero(&WL_avg);
		...
		for (int t : name_field.second.defined_ts()) {
			...
			for (int x = 0; x < L; ++x) {
				for (int y = 0; y < L; ++y) {
					for (int z = 0; z < L; ++z) {
						for (int i = 1; i < 4; ++i) {
							LinkPath S0(gauge_field, T, L, { t + operator_T_Toffset[fieldname].second, x, y, z });
							LinkPath ST(gauge_field, T, L, { t + operator_T_Toffset[fieldname].first + operator_T_Toffset[fieldname].second,
									x, y, z });
							...
							complex curr_WL;
							close_Wilson_loop(&curr_WL, name_field.second.T_at(t, x, y, z, i), S0.path, ST.path);
