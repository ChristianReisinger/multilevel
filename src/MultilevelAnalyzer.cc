...
MultilevelAnalyzer::MultilevelAnalyzer(MultilevelConfig& multilevel_config, int WL_R,
		...
		config(&multilevel_config), WL_R(WL_R), field_compositions(field_compositions), lowest_level_functions(lowest_level_functions) {
}

void MultilevelAnalyzer::compute_sublattice_fields(double** T_fields, const int level) {
	...
	for (int conf = 1; conf <= config_num; ++conf) {
		...
		if (is_lowest)
			config->get(lower_level_fields[0]);
		else {
			std::cerr << "Allocating sublattice fields on config '" << config->config_filename() << "' ... ";
			for (int i = 0; i < lower_level_field_num; ++i)
				T_field_alloc_zero(lower_level_fields[i], 3, config->T / config->thickness(level + 1), config->L);
			std::cerr << "ok\n";
					...
					for (int z = 0; z < config->L; ++z) {
						for (int i = 1; i < 4; ++i) {
							...
							for (int curr_field_index = 0; curr_field_index < field_compositions[level].size(); ++curr_field_index) {
								...
								for (int lower_level_field_index : field_compositions[level][curr_field_index]) {
									...
									if (is_lowest) {
										...
										lowest_level_functions[lower_level_field_index](
												operator_component, lower_level_fields[0], config->T, config->L, curr_t, x, y, z, i, WL_R);
									} else {
								...
								so_pl_eq_so(T_fields[curr_field_index]
		...
		if (!is_lowest)
			for (int i = 0; i < lower_level_field_num; ++i)
				T_field_free(lower_level_fields[i]);
	}
	for (int i = 0; i < field_compositions[level].size(); ++i)
		T_field_di_eq_re(T_fields[i], config_num, 3, config->T, config->L, timeslice_thickness);
