...
MultilevelAnalyzer::MultilevelAnalyzer(MultilevelConfig& multilevel_config, int WL_R,
		...
		config(&multilevel_config), WL_R(WL_R),
...
std::map<std::string, T_field> MultilevelAnalyzer::compute_T_fields() {
	std::map<std::string, T_field> T_fields;
...
void MultilevelAnalyzer::compute_sublattice_fields(std::map<std::string, T_field>& T_fields, const int level) {
	...
	for (int conf = 1; conf <= config_num; ++conf) {
		...
		std::map<std::string, T_field> lower_level_fields;
		...
		for (auto& name_field : T_fields) {
			try {
				for (int t : name_field.second.defined_ts()) {
						...
						for (int y = 0; y < config->L; ++y) {
							for (int z = 0; z < config->L; ++z) {
								for (int i = 1; i < 4; ++i) {
									...
									for (const std::string& factor_name : level_operator_factors.at(level).at(name_field.first)) {
										double* factor;
										if (is_lowest) {
											...
											factor = lower_level_fields.at(factor_name).T_at(curr_t, x, y, z, i);
											curr_t += lower_level_fields.at(factor_name).timeslice_thickness();
										}
									...
									so_pl_eq_so(name_field.second.T_at(t, x, y, z, i), curr_operator);
								}
							}
						}
					}
				...
				throw std::runtime_error("invalid definition of operator '" + name_field.first + "'");
			}
		}
		time_spent_computing_operators += std::chrono::steady_clock::now() - start_time;
	}
	for (auto& name_field : T_fields)
		name_field.second /= config_num;
}

void MultilevelAnalyzer::alloc_T_fields(std::map<std::string, T_field>& T_fields, const int level) {
	for (const auto& fieldname_factors : level_operator_factors.at(level)) {
		...
		T_fields.insert( { name, T_field(thickness_defined, config->T, config->L) });
	}
}
